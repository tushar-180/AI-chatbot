...
import { calculateUsage, estimateTokenCount, serializePromptMessages, logTokenUsage } from "../utils/tokenCounter";
import { cleanupChatFiles } from "../modules/file-rag/fileHandler";
...
export class GroupChatService {
  ...
  static async createGroup(chatId: string, clerkId: string) {
    ...
    if (originalChat.userId !== clerkId) {
      console.error("Unauthorized group creation attempt for chat:", chatId);
      throw new Error("You are not allowed to convert this chat into a group");
    }
  ...
  static async handleAiResponse(
    ...
    try {
      ...
      try {
        ...
        const logUsername = clerkId || group.creatorId;
        const logMember = group.members.find(m => m.userId === logUsername);
        const resolvedLogUsername = logMember?.username || logUsername;

        logTokenUsage({
          model: targetProvider,
          usage: tokens,
          context: "groupChat",
          username: resolvedLogUsername,
