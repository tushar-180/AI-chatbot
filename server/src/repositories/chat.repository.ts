...
export const chatRepository = {
  ...
  async findById(chatId: string) {
    ...
    const legacyMessages = (chat.toObject() as any).messages || (chat.toObject() as any).legacyMessages || [];
  ...
  findAllByUserId(userId: string) {
    return Chat.find({ userId }).select("-messages -legacyMessages").sort({ updatedAt: -1 });
  },
  ...
  async updateMessageByRequestId(chatId: string, requestId: string, updateData: Partial<ChatMessage>) {
    return await Message.findOneAndUpdate({ chatId, requestId }, updateData, { returnDocument: "after" });
  }
};
