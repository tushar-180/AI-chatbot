...
const pipeStreamResponse = async (
    ...
    try {
        ...
        console.error("pipeStreamResponse: Uncaught generator error:", error);
        endSse(res, { error: "Stream interrupted" });
    }
};
...
export const createChatStream = async (req: Request, res: Response) => {
    try {
        await pipeStreamResponse(
            req,
            res,
            chatService.createChatStream(req.body),
        );
    } catch (error) {
...
export const getGallery = asyncHandler(async (req: Request, res: Response) => {
    try {
        const gallery = await chatService.getGallery(
            String(req.params.id || req.params.userId),
        );
