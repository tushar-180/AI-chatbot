...
const SettingsModal: React.FC<SettingsModalProps> = ({
  ...
  const { deleteChat: globalDeleteChat, unarchiveChat: globalUnarchiveChat } =
    useChatList();
  const {
    setCurrentChat,
    setMessages,
  ...
  const { removeGroup, currentGroupId, setCurrentGroup } = useGroupStore();
  ...
  const [isMcpActionLoading, setIsMcpActionLoading] = useState<string | null>(
    null,
  );
  ...
  const toggleMcpServer = async (name: string, currentEnabled: boolean) => {
    ...
    try {
      ...
      const { data } = await api.patch(`/mcp/${name}/toggle`, {
      ...
      setMcpServers((prev) =>
        prev.map((s) =>
          s.name === name
            ? { ...s, enabled: data.server.enabled, connected: data.connected }
            : s,
        ),
      );
    } catch (error) {
  ...
  const reconnectMcpServer = async (name: string) => {
    ...
    try {
      ...
      setMcpServers((prev) =>
        prev.map((s) =>
          s.name === name
            ? { ...s, ...data.server, connected: data.connected }
            : s,
        ),
      );
  ...
  const [deleteType, setDeleteType] = useState<
    "archive" | "share" | "group" | "memory" | null
  >(null);
  ...
  const handleConfirmDelete = async () => {
    ...
    if (deleteType === "archive") {
      try {
        ...
        setLocalArchivedChats((prev) =>
          prev.filter((c) => !ids.includes(c._id)),
        );
        toast.success(
          ids.length > 1 ? "Archived chats deleted" : "Archived chat deleted",
        );
      } catch (error) {
      ...
      try {
        ...
        toast.success(
          ids.length > 1
            ? "Shared chat links deleted"
            : "Shared chat link deleted",
        );
      } catch (error) {
      ...
      try {
        await Promise.all(
          ids.map((id) =>
            api.delete(`/group/${id}`, { data: { userId: user?.id } }),
          ),
        );
        setCreatedGroups((prev) => prev.filter((g) => !ids.includes(g._id)));
        ids.forEach((id) => removeGroup(id));
        toast.success(
          ids.length > 1 ? "Group chats deleted" : "Group chat deleted",
        );
  ...
  const modalContent = (
    ...
    >
      <div
        ...
        className="w-full max-w-4xl h-full lg:h-[600px] max-h-[100vh] lg:max-h-[85vh] bg-zinc-950 border lg:border-zinc-800/60 rounded-none lg:rounded-3xl shadow-2xl overflow-hidden flex flex-col lg:flex-row animate-in zoom-in-95 duration-300 relative"
      >
        {/* Internal Sidebar */}
        <div
          className={`w-full lg:w-64 bg-zinc-900/50 border-b lg:border-b-0 lg:border-r border-zinc-800/40 p-4 flex-col shrink-0 ${mobileView === "list" ? "flex" : "hidden lg:flex"}`}
        >
          <div className="mb-6 lg:mb-8 px-2 flex items-center justify-between">
            ...
            <button
              onClick={onClose}
              className="lg:hidden p-2 hover:bg-white/5 rounded-xl transition-all text-zinc-500 hover:text-white"
            >
              <X size={20} />
            </button>
          </div>
          ...
          <div className="lg:hidden flex-1 overflow-y-auto space-y-2 py-2">
            {navItems.map((item) => (
              <button
                ...
                className="flex w-full items-center justify-between rounded-2xl bg-white/2 border border-zinc-800/40 px-4 py-3.5 text-xs font-bold uppercase tracking-widest text-zinc-300 hover:bg-white/5 hover:text-white transition-all active:scale-[0.99]"
              >
          ...
          {/* Desktop SignOut */}
          <div className="hidden lg:block mt-auto pt-4 border-t border-zinc-800/40">
            <SignOutButton>
          ...
          {/* Mobile SignOut */}
          <div className="lg:hidden mt-auto pt-4 border-t border-zinc-800/40">
            <SignOutButton>
        ...
        {/* Content Area */}
        <div
          className={`flex-1 flex flex-col min-w-0 bg-zinc-950 ${mobileView === "detail" ? "flex" : "hidden lg:flex"}`}
        >
          {/* Header (Mobile Close) */}
          <div className="flex items-center justify-between px-4 lg:px-8 py-4 lg:py-6 border-b border-zinc-800/40 gap-4">
            <div className="flex items-center gap-3 min-w-0">
              <button
                onClick={() => setMobileView("list")}
                className="lg:hidden p-2 hover:bg-white/5 rounded-xl transition-all text-zinc-400 hover:text-white shrink-0 animate-in slide-in-from-left-2 duration-200"
              >
                <ArrowLeft size={18} />
              </button>
            ...
            <button
              onClick={onClose}
              className="p-2 hover:bg-white/5 rounded-xl transition-all text-zinc-500 hover:text-white shrink-0"
            >
          ...
          <div className="flex-1 overflow-y-auto p-4 lg:p-8">
            {activeTab === "general" && !isEditingProfile && (
              <div className="space-y-6 lg:space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between p-4 lg:p-6 rounded-3xl bg-zinc-900/30 border border-zinc-800/40 overflow-hidden group gap-4">
                  <div className="flex items-center gap-4 lg:gap-6">
                    <div className="h-16 w-16 lg:h-20 lg:w-20 rounded-2xl overflow-hidden border-2 border-zinc-800/60 shadow-2xl shrink-0">
                      <img
                    ...
                    <div className="min-w-0">
                      <h4 className="text-base lg:text-xl font-bold text-white truncate">
                        {dbUser?.firstName
                          ? `${dbUser.firstName} ${dbUser.lastName || ""}`
                          : user?.fullName}
                      </h4>
                      ...
                      <div className="mt-2 flex flex-wrap items-center gap-2">
                        <span className="px-2 py-0.5 rounded-lg bg-emerald-500/10 text-emerald-400 text-[10px] font-bold uppercase tracking-widest border border-emerald-500/20">
                          Premium
                        </span>
                        {/* eslint-disable-next-line react-hooks/purity */}
                        <span className="text-zinc-700 text-[10px] font-medium">
                          • Member since{" "}
                          {new Date(
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-white/5 border border-zinc-800/40 text-zinc-400 hover:text-white hover:bg-white/10 text-[9px] font-bold uppercase tracking-widest transition-all shrink-0 self-start sm:self-center"
                  >
                    <Pencil size={12} />
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="p-4 lg:p-5 rounded-3xl border border-zinc-800/40 bg-white/2 hover:bg-white/5 transition-colors cursor-pointer group">
                  ...
                  <div className="p-4 lg:p-5 rounded-3xl border border-zinc-800/40 bg-white/2 hover:bg-white/5 transition-colors cursor-pointer group">
                    ...
                    <p className="text-sm font-medium text-white">
                      {personalizationData.occupation || "Not Set"}
                    </p>
                  </div>
                </div>
              </div>
            ...
            {activeTab === "general" && isEditingProfile && (
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                <div className="flex flex-col items-center justify-center p-4 lg:p-6 rounded-3xl bg-zinc-900/30 border border-zinc-800/40 gap-4 relative">
                  <input
                    type="file"
                    ref={fileInputRef}
                  ...
                  <div
                    ...
                    className="h-20 w-20 lg:h-24 lg:w-24 rounded-3xl overflow-hidden border-2 border-zinc-800/60 shadow-2xl relative cursor-pointer group/avatar"
                  >
                    <img
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 lg:gap-6">
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      className="w-full bg-zinc-900/30 border border-zinc-800/40 rounded-2xl px-4 py-3 text-sm text-white focus:outline-none focus:ring-2 focus:ring-white/10 transition-all"
                    />
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest ml-1">
                ...
                <div className="flex flex-col sm:flex-row gap-3 pt-2">
                  <button
                    ...
                    className="w-full sm:flex-1 py-3 lg:py-4 rounded-2xl text-[11px] font-bold uppercase tracking-widest bg-white/5 border border-zinc-800/40 text-zinc-400 hover:text-white hover:bg-white/10 transition-all disabled:opacity-50"
                  >
                    Cancel
                  </button>
                  <button
                    ...
                    className="w-full sm:flex-1 flex items-center justify-center gap-2 bg-white text-black py-3 lg:py-4 rounded-2xl text-[11px] font-bold uppercase tracking-widest hover:bg-zinc-200 transition-all disabled:opacity-50"
                  >
                    {isSavingProfile ? (
                  ...
                  <>
                    ...
                    <div className="space-y-2">
                      ...
                      <textarea
                        ...
                        className="w-full bg-zinc-900/30 border border-zinc-800/40 rounded-2xl px-4 py-3 text-sm text-white focus:outline-none focus:ring-2 focus:ring-white/10 transition-all resize-none lg:rows-4"
                      />
                    </div>
                    ...
                    <button
                      ...
                      className="w-full flex items-center justify-center gap-2 bg-white text-black py-3 lg:py-4 rounded-2xl text-[11px] font-bold uppercase tracking-widest hover:bg-zinc-200 transition-all disabled:opacity-50"
                    >
            ...
            {activeTab === "memory" && (
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                ...
                {isMemoryLoading ? (
                  ...
                  <div className="space-y-3">
                    {isEditMode && (
                      <div className="flex flex-wrap items-center justify-between p-3 rounded-2xl bg-white/2 border border-zinc-800/40 mb-3 animate-in fade-in slide-in-from-top-1 duration-200 gap-2">
                        <button
                          onClick={() => {
                          ...
                          className="px-3 py-1.5 rounded-xl bg-white/5 text-zinc-300 hover:text-white hover:bg-white/10 border border-zinc-800/40 transition-all text-[9px] font-bold uppercase tracking-widest"
                        >
                          {memories.every((m) => selectedIds.includes(m._id))
                            ? "Deselect All"
                            : "Select All"}
                        </button>
                    ...
                    {memories.map((memory) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="mt-1 text-zinc-600 group-hover:text-emerald-400 transition-colors">
                            <Zap size={14} />
                          </div>
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-2 mt-2">
                              <span className="text-[9px] font-bold upabortControllerRefpercase tracking-widest text-zinc-600">
                                {memory.category}
                              </span>
                          ...
                          {!isEditMode && (
                            <button
                              ...
                              className="opacity-0 group-hover:opacity-100 p-2 text-zinc-600 hover:text-rose-500 transition-all cursor-pointer"
                            >
                              <Trash2 size={14} />
                            </button>
                          )}
                    ...
                    {hasMoreMemory && (
                      <button
                        ...
                        className="w-full py-3 text-[10px] font-bold uppercase tracking-widest text-zinc-500 hover:text-white transition-colors"
                      >
            ...
            {activeTab === "archive" && (
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-amber-400/60 font-medium">
                        {localArchivedChats.length} Conversations preserved
                      </p>
                    </div>
                  </div>
                ...
                {isArchiveLoading ? (
                  ...
                  <div className="text-center py-20 bg-white/2 border border-dashed border-zinc-800/40 rounded-3xl">
                    <History className="w-10 h-10 text-zinc-800 mx-auto mb-4 opacity-50" />
                    <p className="text-zinc-500 text-[10px] font-bold uppercase tracking-widest">
                      The vault is empty
                    </p>
                  </div>
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-3 flex-1 min-w-0 mr-4">
                            ...
                            <div className="h-10 w-10 rounded-xl bg-zinc-900 flex items-center justify-center text-zinc-500 group-hover:text-amber-400 transition-colors shrink-0">
                          ...
                          {!isEditMode && (
                            <div className="flex items-center gap-2 shrink-0">
                              <button
                                ...
                                className="p-2.5 rounded-xl bg-white/5 text-zinc-400 hover:text-emerald-400 hover:bg-emerald-500/10 transition-all flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-widest"
                              >
                                <ArrowLeft size={14} />
                              ...
                              <button
                                ...
                                className="p-2.5 rounded-xl bg-white/5 text-zinc-400 hover:text-rose-400 hover:bg-rose-500/10 transition-all"
                              >
                                <Trash2 size={14} />
                              </button>
                            </div>
                ...
                {isSharingLoading ? (
                  ...
                  <div className="space-y-6">
                    <div className="space-y-3">
                      ...
                      {sharedChats.length === 0 ? (
                        <div className="p-8 text-center bg-white/2 border border-dashed border-zinc-800/40 rounded-2xl">
                          <p className="text-zinc-500 text-[10px] font-bold uppercase tracking-widest">
                            No shared chats yet
                          </p>
                        </div>
                            ...
                            return (
                              ...
                              >
                                <div className="flex items-center gap-3 flex-1 min-w-0 mr-4">
                                  ...
                                  <div className="min-w-0 flex-1">
                                    <p className="text-sm font-bold text-white truncate">
                                      {share.title || "Shared Chat Session"}
                                    </p>
                                ...
                                {!isEditMode && (
                                  <div className="flex items-center gap-2 shrink-0">
                                    ...
                                    <button
                                      onClick={() =>
                                        promptDeleteShare([share._id])
                                      }
                                      className="p-2.5 rounded-xl bg-white/5 border border-transparent text-zinc-400 hover:text-rose-400 hover:bg-rose-500/10 hover:border-rose-500/20 transition-all"
                ...
                <div className="p-4 rounded-2xl bg-indigo-500/5 border border-indigo-500/10 mb-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-indigo-400/60 font-medium">
                        Manage and delete collaborative groups you created
                      </p>
                    </div>
                  </div>
                        ...
                        <div className="space-y-2">
                          {isEditMode && (
                            <div className="flex flex-wrap items-center justify-between p-3 rounded-2xl bg-white/2 border border-zinc-800/40 mb-3 animate-in fade-in slide-in-from-top-1 duration-200 gap-2">
                              <button
                                onClick={() => {
                                  const allIds = createdGroups.map(
                                    (c) => c._id,
                                  );
                                  const allSelected = allIds.every((id) =>
                                    selectedIds.includes(id),
                            ...
                            return (
                              ...
                              >
                                <div className="flex items-center gap-3 flex-1 min-w-0 mr-4">
                                  ...
                                  <div className="min-w-0 flex-1">
                                    <p className="text-sm font-bold text-white truncate">
                                      {group.title || "Collaborative Group"}
                                    </p>
                                    <div className="flex flex-wrap items-center gap-2 mt-1">
                                      <span className="text-[9px] text-indigo-400 font-bold bg-indigo-500/10 px-2 py-0.5 rounded-md">
                                        Creator
                                      </span>
                                      <span className="text-[9px] text-zinc-500 font-medium">
                ...
                <div className="p-4 lg:p-6 rounded-3xl bg-blue-500/5 border border-blue-500/10 mb-4">
                  <div className="flex items-center gap-3 lg:gap-4 mb-3 lg:mb-4">
                    ...
                    <div>
                      ...
                      <p className="text-[9px] lg:text-[10px] text-blue-400/60 font-bold uppercase tracking-widest">
                        Context Export System
                      </p>
                    </div>
                  </div>
                  <p className="text-xs lg:text-sm text-zinc-400 leading-relaxed">
                    Generate a comprehensive summary of everything Velora has
                    learned about you. This export is designed to help you
                    "port" your context to other AI assistants, preserving your
                    instructions, preferences, and key life events.
                  </p>
                </div>
                ...
                {!exportSummary ? (
                  <div className="space-y-3">
                    <button
                      ...
                      className="w-full flex items-center justify-center gap-3 bg-white text-black py-5 rounded-3xl text-[12px] font-bold uppercase tracking-[0.2em] hover:bg-zinc-200 transition-all disabled:opacity-50"
                    >
                      {isExportLoading ? (
                  ...
                  <div className="space-y-4">
                    <div className="relative group">
                      <div className="absolute top-4 right-4 flex items-center gap-2 opacity-100 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity z-10">
                        ...
                        <button
                          onClick={() => {
                            const blob = new Blob([exportSummary], {
                              type: "text/plain",
                            });
                      ...
                      <pre className="w-full h-[300px] lg:h-[350px] overflow-y-auto bg-black/40 border border-zinc-800/40 rounded-2xl lg:rounded-3xl p-4 lg:p-8 text-[11px] lg:text-xs text-zinc-300 font-mono leading-relaxed whitespace-pre-wrap selection:bg-white/10">
                        {exportSummary}
                      </pre>
                    </div>
                    <button
                      onClick={() => setExportSummary("")}
                      className="w-full py-4 text-[10px] font-bold uppercase tracking-widest text-zinc-500 hover:text-white transition-colors"
                    >
            ...
            {activeTab === "security" && (
              <div className="space-y-6 lg:space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
                <div className="space-y-4">
                  <h4 className="text-[10px] lg:text-[11px] font-bold text-zinc-500 uppercase tracking-[0.2em]">
                    Security Controls
                  </h4>
                  <div className="space-y-3">
                    <div className="flex items-center justify-between p-4 lg:p-5 rounded-2xl lg:rounded-3xl bg-zinc-900/30 border border-zinc-800/40">
                      <div>
                        <p className="text-xs lg:text-sm font-bold text-white">
                          Encryption
                        </p>
                        <p className="text-[9px] lg:text-[10px] text-zinc-500 font-medium">
                          All neural data is end-to-end encrypted
                        </p>
                      </div>
                    ...
                    <div className="flex items-center justify-between p-4 lg:p-5 rounded-2xl lg:rounded-3xl bg-zinc-900/30 border border-zinc-800/40">
                      <div>
                        ...
                        <p className="text-[9px] lg:text-[10px] text-zinc-500 font-medium">
                          Manage how your data is used for training
                        </p>
                      </div>
                ...
                <div className="pt-6 border-t border-zinc-800/40">
                  <p className="text-[10px] font-bold text-rose-500/60 uppercase tracking-[0.2em] mb-4">
                    Danger Zone
                  </p>
                  <button className="flex items-center gap-3 w-full p-4 lg:p-5 rounded-2xl lg:rounded-3xl border border-rose-500/20 bg-rose-500/5 text-rose-500 hover:bg-rose-500/10 transition-all text-left">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] opacity-60 font-medium">
                        Permanently wipe all neural and chat history
                      </p>
                    </div>
                  </button>
                </div>
              ...
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                ...
                <div className="p-5 rounded-3xl bg-amber-500/5 border border-amber-500/10 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-amber-400/60 font-medium">
                        Extend Velora with real-time web, data, and developer
                        actions
                      </p>
                    </div>
                  </div>
                ...
                {isMcpLoading ? (
                  <div className="flex flex-col items-center justify-center py-20 gap-3">
                    <Loader2 className="w-6 h-6 text-amber-500 animate-spin" />
                    <p className="text-[10px] uppercase tracking-widest text-zinc-600 font-bold">
                      Synchronizing Active Integrations...
                    </p>
                  </div>
                ) : mcpError ? (
                  <div className="text-center py-20 bg-white/2 border border-dashed border-zinc-800/40 rounded-3xl space-y-3">
                    <WifiOff className="w-10 h-10 text-zinc-800 mx-auto opacity-50 animate-pulse" />
                    <p className="text-rose-400 text-[10px] font-bold uppercase tracking-widest">
                      {mcpError}
                    </p>
                    <button
                      onClick={fetchMcpServers}
                      className="text-[9px] font-bold text-zinc-400 hover:text-white uppercase tracking-widest border border-zinc-800/40 px-3 py-1.5 rounded-xl hover:bg-white/5"
                    >
                      Retry Connection
                    </button>
                  </div>
                  ...
                  <div className="text-center py-20 bg-white/2 border border-dashed border-zinc-800/40 rounded-3xl">
                    <Zap className="w-10 h-10 text-zinc-800 mx-auto mb-4 opacity-50" />
                    <p className="text-zinc-500 text-[10px] font-bold uppercase tracking-widest">
                  ...
                  <div className="space-y-4">
                    {mcpServers.map((server) => {
                      ...
                      if (
                        serverNameLower.includes("search") ||
                        serverNameLower.includes("web")
                      ) {
                        IconComponent = Globe;
                      } else if (
                        serverNameLower.includes("sqlite") ||
                        serverNameLower.includes("db") ||
                        serverNameLower.includes("sql")
                      ) {
                        IconComponent = Database;
                      } else if (
                        serverNameLower.includes("github") ||
                        serverNameLower.includes("git")
                      ) {
                      ...
                      return (
                        <div
                          key={server.name}
                          className="group rounded-3xl border transition-all p-5 space-y-4 bg-zinc-900/40 border-zinc-800/40 hover:border-zinc-800/60"
                        >
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                ...
                                <div className="flex items-center gap-2 mt-1">
                                  ...
                                  <span className="text-zinc-700 text-[9px] font-medium">
                                    •
                                  </span>
                                  <span className="text-zinc-600 text-[9px] font-semibold uppercase tracking-widest font-mono">
                                    {server.type}
                                  </span>
                                </div>
                              </div>
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              {server.enabled && (
                                <button
                                  ...
                                  className="p-1.5 rounded-xl bg-white/5 border border-zinc-800/40 text-zinc-400 hover:text-amber-400 hover:bg-amber-500/10 hover:border-amber-500/20 transition-all flex items-center justify-center cursor-pointer"
                              ...
                              {isMcpActionLoading === server.name ? (
                                <Loader2
                                  size={16}
                                  className="animate-spin text-zinc-500 mr-2"
                                />
                              ) : (
                                <button
                                  onClick={() =>
          ...
          {deleteModalOpen && (
            <div className="absolute inset-0 z-50 flex items-center justify-center bg-zinc-950/80 backdrop-blur-sm animate-in fade-in duration-300">
              <div className="w-full max-w-sm p-8 rounded-3xl bg-zinc-900/90 border border-zinc-800/60 shadow-[0_0_50px_rgba(0,0,0,0.8)] space-y-6 text-center animate-in zoom-in-95 duration-200">
                ...
                <div className="flex items-center gap-3 pt-2">
                  <button
                    ...
                    className="flex-1 py-3 rounded-2xl bg-white/5 border border-zinc-800/40 hover:bg-white/10 hover:text-white text-zinc-300 text-xs font-bold uppercase tracking-wider transition-all"
                  >
                    Cancel
                  </button>
                  <button
