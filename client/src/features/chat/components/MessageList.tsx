...
const MessageList = ({
    ...
    const messageIds = useMessageStore(
        useShallow((state) => {
            const msgs =
                state.messagesByChatId[currentChatId || "__temp__"] || [];
            return msgs.map((m) => m.id as string);
        }),
    );
    ...
    // Handle new messages and chat switches
    useEffect(() => {
        const messageCountChanged =
            messageIds.length !== previousMessageCountRef.current;
    ...
    // Handle height changes (e.g. streaming text)
    useEffect(() => {
        const container = scrollContainerRef.current?.firstElementChild;
        if (!container) return;

        const resizeObserver = new ResizeObserver(() => {
            if (shouldAutoScrollRef.current && isStreaming) {
                scrollToBottom(true); // use instant scroll for rapid stream updates
            }
        });
    ...
    const showSuggestions = !currentChatId && messageIds.length === 0;

    // Optional: detect if the last message is from user to show loading pulse before assistant starts
    const lastMessage = useMessageStore((state) => {
        const msgs = state.messagesByChatId[currentChatId || "__temp__"];
    ...
    return (
        ...
        >
            <div className="mx-auto max-w-5xl flex flex-col gap-7">
                {showSuggestions ? (
                  ...
                  messageIds.length === 0 ? (
                ...
                ) : messageIds.length === 0 && !loading && !isStreaming ? (
                    ...
                    <>
                        ...
                        {showScrollToBottom && messageIds.length > 0 && (
