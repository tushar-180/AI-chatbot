...
export const useChatStream = (hookOptions?: {
    ...
    const flushOptimisticUpdates = () => {
        ...
        setOptimisticMessagesByChatId((current) => {
            ...
            for (const key of Object.keys(pendingUpdates)) {
                ...
                if (!lastMessage || lastMessage.id !== pending.messageId)
                    continue;
            ...
            if (data.chatId) {
                ...
                if (initialKey === NEW_CHAT_STREAM_KEY) {
                    ...
                    if (
                        ...
                        delete pendingOptimisticUpdateRef.current[
                            NEW_CHAT_STREAM_KEY
                        ];
                    }
                }
    ...
    const streamMessage = async (
        ...
        try {
            const url = chatService.getStreamUrl(
                effectiveCurrentChatId || undefined,
            );
            ...
            if (!response.ok) {
                if (
                    ...
                    const resolvedChatId =
                        activeResolvedChatIdRef.current ??
