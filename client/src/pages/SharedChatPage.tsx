...
export default function SharedChatPage() {
  ...
  useEffect(() => {
    const fetchSharedChat = async () => {
      try {
        ...
        const msgs = (res.data.messages || []).map((msg: any, i: number) => ({
          id: `shared-${i}`,
          role: msg.role,
          content: msg.content,
        }));
        ...
        setError(err.response?.data?.error || "Failed to load shared chat");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleFork = async (
    ...
    try {
      ...
      const forkedMessages: Message[] = sharedMessages.map((msg, index) => ({
  ...
  if (error) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-[#030712] p-4 text-center">
        <h2 className="mb-4 text-2xl font-bold text-white">Chat Not Found</h2>
